...
async function extractLinks(pdfUrl) {
  ...
  for (let p = 1; p <= pdf.numPages; p++) {
    ...
    // Debug: page.getTextContent().items
    const debugArray = [];
    if (debugArray.includes(p)) {
      ...
      items.forEach((item, i) => {
        ...
        console.log(`  [${i}] x=${x} y=${y} w=${w} | ${JSON.stringify(str)} | ${fontName}`);
      });
      console.groupEnd();
    }

    const urlsFromPage = concatTextContent(items);
